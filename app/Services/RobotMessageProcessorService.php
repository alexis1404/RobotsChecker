            ...
            $excel->sheet('Данные аудита', function($sheet) use ($check_status){
                ...
                if($check_status['statusfail'] && !$check_status['no_robots']) {
                    $sheet->row(3, [
                        ...
                        'Рекомендации (программист): файл robots.txt должен отдавать код ответа 200, иначе файл не будет обрабатываться. Необходимо настроить сайт таким образом, чтобы при обращении к robots.txt сервер возвращал ответ 200'
                    ]);
                ...
                if($check_status['hosts_dir'] && $check_status['hosts_dir'] == 1 && !$check_status['no_robots']){
                    ...
                    $sheet->row(7, [
                        ...
                        'Рекомендации (программист): Директива Host должна быть указана в файле только один раз. Необходимо удалить все дополнительные директивы Host оставив только одну (1) корректную и соответствующую основному зеркалу сайта.'
                    ]);
                ...
                if(!$check_status['no_robots'] && $check_status['file_size'] < 3200){
                    ...
                    $sheet->row(9, [
                        ...
                        'Рекомендации (программист): Максимально допустимый размер файла robots.txt составляет 32кб. Необходимо отредактировать файл rosots.txt таким образом, чтобы его размер не превышал 32кб.'
                    ]);
